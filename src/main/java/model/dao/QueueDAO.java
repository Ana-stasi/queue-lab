...
public class QueueDAO extends BaseDAO{
    ...
    public Queue getQueue(Student student, Subject subject){
        String getQueueLine = "select s.name,surname,date from queue inner join students s on " +
                "s.student_id = queue.student_id inner join groups g on s.group_id = g.group_id " +
                "where subject_id in (select subject_id from subject where name = ?) and g.group_id " +
                "in (select g.group_id from groups where g.name = ?)";
        Queue queue = null;
